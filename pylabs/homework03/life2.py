...
class CellList:
    ...
    def update(self):
        new_cell_list = deepcopy(self.cell_grid)
        for i in range(len(self.cell_grid)):
            for j in range(len(self.cell_grid[0])):
                ...
                if 2 <= summary <= 3 and self.cell_grid[i][j].life:
                    new_cell_list[i][j].life = 1
                elif summary == 3 and not self.cell_grid[i][j].life:
                    new_cell_list[i][j].life = 1
                else:
                    new_cell_list[i][j].life = 0
        self.cell_grid = new_cell_list
    ...
    def get_neighbours(self, cell):
        ...
        for r, c in positions:
            if 0 <= cell[0] + r < self.nrows and 0 <= cell[1] + c < self.ncols:
                neighbours.append(self.cell_grid[cell[0] +
                                                 r][cell[1] + c].life)
        return neighbours
    ...
    def from_file(cls, filename):
        with open(filename) as f:
            new_grid = [[Cell(i, j, int(value)) for j, value in enumerate(line)
                         if value in '01'] for i, line in
                        enumerate(f)]
    ...
    def __next__(self):
        if self.row_num == self.nrows:
            raise StopIteration
        cell = self.cell_grid[self.row_num][self.col_num]
        self.col_num += 1
...
class GameOfLife:
    ...
    def run(self):
        ...
        pygame.display.set_caption('PSYCHEDELIC by Elizabeth ;)')
